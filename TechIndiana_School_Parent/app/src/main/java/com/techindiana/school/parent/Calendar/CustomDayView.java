package com.techindiana.school.parent.Calendar;
        ...
        if (Utils.loadMarkData().containsKey(date.toString())) {
            if (state == State.SELECT || date.toString().equals(today.toString())) {
                ...
                if (Utils.loadMarkData().get(date.toString()).equals("0")) {
                    ...
                    marker.setBackgroundResource(R.drawable.view_syllabus_active1_mark_background);
                } else if (Utils.loadMarkData().get(date.toString()).equals("1")) {
                    // marker.setEnabled(true);
                    marker.setBackgroundResource(R.drawable.view_syllabus_active_mark_background);
                } else {
                    marker.setBackgroundResource(R.drawable.view_syllabus_unactive_mark_background);
                }
            }
        } else {
            marker.setVisibility(GONE);
