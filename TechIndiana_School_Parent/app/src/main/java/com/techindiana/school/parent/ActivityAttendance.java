...
import com.techindiana.school.parent.Adapter.AdapterAttendanceChart;
import com.techindiana.school.parent.Calendar.CustomDayView;
import com.techindiana.school.parent.Module.AttendanceInfo;
...
public class ActivityAttendance extends BaseActivity {
    ...
    ArrayList<AttendanceInfo> attendanceInfos;

    RecyclerView rvChart;
    RecyclerView.LayoutManager managerChart;
    RecyclerView.Adapter adapterChart;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            actionBar.setTitle(getResources().getString(R.string.mAttendance));
        ...
        Methods.setFont(ActivityAttendance.this, viewGroup);
        try {
            Declaration();
    ...
    private void Declaration() {
        try {
            ...
            rvChart = (RecyclerView) findViewById(R.id.list);
            managerChart = new LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false);
            rvChart.setLayoutManager(managerChart);
    ...
    private void getDataOfMonth(String month, String Year) {
        try {
            ...
            Call<AttendanceResp> call = restInterface.getAttendance(Constant.apiKey, Constant.childSchoolId, Constant.childClassId,
                    Constant.childDivId, month, Year, Constant.childID);
            call.enqueue(new Callback<AttendanceResp>() {

                @Override
                public void onResponse(Call<AttendanceResp> call, Response<AttendanceResp> response) {

                    if (response.isSuccessful()) {
                        if (response.body().getSuccess().equals(Constant.RESP_SUCCESS)) {
                            try {
                                attendanceInfos = (ArrayList<AttendanceInfo>) response.body().getAttendanceInfo();
                                ...
                                for (int i = 0; i < attendanceInfos.size(); i++) {
                                    ...
                                    try {
                                        date = inputFormat.parse(attendanceInfos.get(i).getDate());
                                        str = outputFormat.format(date);
                                    } catch (ParseException e) {
                                        e.printStackTrace();
                                    }
                                    if(attendanceInfos.get(i).getStatus().equals("1"))
                                    markData.put(str, "1");
                                    else  if(attendanceInfos.get(i).getStatus().equals("0"))
                                    markData.put(str, "2");
                                    else
                                    markData.put(str, "0");
                                ...
                                ArrayList<HashMap<String, String>> aListChart = new ArrayList<HashMap<String, String>>();
                                HashMap<String, String> h1 = new HashMap<String, String>();
                                h1.put("present", "" + response.body().getPresent());
                                h1.put("holiday", "" + response.body().getHoliday());
                                h1.put("absent", "" + response.body().getAbsent());
                                h1.put("totalDays", "" + response.body().getNoOfDays());
                                aListChart.add(h1);
                              if(aListChart.size()>0) {
                                    AdapterAttendanceChart managerChart = new AdapterAttendanceChart(context, aListChart);
                                    rvChart.setAdapter(managerChart);
                                 //   rvChart.setVisibility(View.VISIBLE);
                                }else{
                                   // rvChart.setVisibility(View.INVISIBLE);
                                }
                                // setData("" + response.body().getPresent(), "" + response.body().getHoliday(), "" + response.body().getAbsent());
                            ...
                            rvChart.setAdapter(managerChart);
                            //rvChart.setVisibility(View.INVISIBLE);
                            Toast.makeText(ActivityAttendance.this, "" + response.body().getMessage(), Toast.LENGTH_SHORT).show();
                        }
                    }
                    hideProgress();
                }
                ...
                @Override
                public void onFailure(Call<AttendanceResp> call, Throwable t) {
                    hideProgress();
                }
            });
        } catch (Exception e) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case android.R.id.home:
                ...
                break;
            case R.id.actionLib:
                intent = new Intent(ActivityAttendance.this, ActivityLibBooks.class);
                startActivity(intent);
                break;
            case R.id.actionNotification:
                intent = new Intent(ActivityAttendance.this, ActivityNotification.class);
                startActivity(intent);
                break;
        }
